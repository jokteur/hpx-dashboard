...
class HPXParser:
    ...
    def __init__(self, out_file=None, print_out=False, strip_hpx_counters=False):
    ...
    def parse_line(self, line):
        ...
        if result:
            ...
            if len(line_split) in [5,6]:
                ...
                if not fullname in self.data:
                    self.data[fullname] = {}

                if full_instancename and not full_instancename in self.data[fullname]:
                    self.data[fullname][full_instancename] = {
                        'parameters': [parameters],
                        'data': {'sequence_number': [], 'timestamp': [], 'timestamp_unit': [], 'value': [], 'value_unit': []}
                    }
                ...
                self.data[fullname][full_instancename]['data']['sequence_number'].append(int(line_split[1]))
                self.data[fullname][full_instancename]['data']['timestamp'].append(line_split[2])
                self.data[fullname][full_instancename]['data']['timestamp_unit'].append(line_split[3])
                self.data[fullname][full_instancename]['data']['value'].append(line_split[4])
                self.data[fullname][full_instancename]['data']['value_unit'].append(value_unit)
