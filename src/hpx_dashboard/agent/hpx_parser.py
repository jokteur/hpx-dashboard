...
class HPXParser:
    ...
    async def parse_line(self, line, queue):
        ...
        if is_line_hpx_counter_data:
            ...
            if self.collect_counter_infos:
                ...
                await queue.put(pickle.dumps(("counter-infos", self.counter_descriptions)))
        ...
        if len(split) > 1:
            ...
            if self.collect_counter_infos and "fullname" in split[0] and re.match(regex, split[1]):
    ...
    async def start_collection(self, input_stream, queue):
        ...
        await queue.put(pickle.dumps(("transmission_begin", time.time())))
        ...
        await queue.put(pickle.dumps(("transmission_end", time.time())))
